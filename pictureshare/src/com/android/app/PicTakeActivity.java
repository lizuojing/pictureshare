...
//		list = PicService.getCurrrentBlocks();
	...
	private Bitmap getBitmap(String filePath) {
		...
		try {
			return ImageUtil.getThumbnail(filePath, width, width,
					photoOrientation);
		...
		try {
			...
			if (Utils.isNotNullOrEmpty(filePath)) {
				...
				bitmap = BitmapFactory.decodeResource(getResources(),
						R.drawable.android_default);
				ImageLoaderManager imageLoaderManager = new ImageLoaderManager(
						this, new Handler(), imageView);
				imageView.setImageBitmap(imageLoaderManager.getImage(url,
						R.drawable.android_default));
			}
...
//		listView.setAdapter(listAdapter);
	...
	class ListAdapter extends BaseAdapter {
		...
		public ListAdapter(ArrayList<Block> list) {
			imageLoaderManager = new ImageLoaderManager(PicTakeActivity.this,
					new Handler(), this);
			this.list = list;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			MenueItem item = null;
			if (convertView == null) {
				item = new MenueItem(PicTakeActivity.this, block,
						imageLoaderManager);
				convertView = item;
			} else {
