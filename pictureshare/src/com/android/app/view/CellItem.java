...
public class CellItem extends RelativeLayout {
	...
	private void createLayout(Context context) {
		...
		/*
		 * RelativeLayout.LayoutParams rightLP = new
		 * RelativeLayout.LayoutParams(
		 * LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
		 * rightLP.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
		 * rightLP.addRule(RelativeLayout.CENTER_VERTICAL);
		 * rightLP.setMargins(Utils.dipToPixels(context, 10), 0,
		 * Utils.dipToPixels(context, 10), 0); ImageView rightView = new
		 * ImageView(context);
		 * rightView.setImageResource(R.drawable.android_default); mRightImage =
		 * rightView; addView(rightView,rightLP);
		 */
	}
