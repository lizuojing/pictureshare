...
class AccessTokenManager implements Parcelable {
	...
	public boolean isSessionKeyValid() {
		if (sessionKey != null && sessionSecret != null && System.currentTimeMillis() < expireTime) {
			return true;
		}
	...
	private void compareWithConfig() {
		...
		final SharedPreferences sp = context.getSharedPreferences(RENREN_SDK_CONFIG, Context.MODE_PRIVATE);
		sp.registerOnSharedPreferenceChangeListener(new OnSharedPreferenceChangeListener() {

			@Override
			public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
				String sKey = sp.getString(RENREN_SDK_CONFIG_PROP_SESSION_KEY, null);
				String secret = sp.getString(RENREN_SDK_CONFIG_PROP_SESSION_SECRET, null);
				long userId = sp.getLong(RENREN_SDK_CONFIG_PROP_USER_ID, 0);
				if ((sessionKey != null && !sessionKey.equals(sKey))
						|| (sessionSecret != null && !sessionKey.equals(secret) || (sessionKey == null && sKey != null) || (sessionSecret == null && secret != null))
						|| (uid != userId)) {
					initSessionKey();
				}
			}
	...
	void clearPersistSession() {
		Editor editor = context.getSharedPreferences(RENREN_SDK_CONFIG, Context.MODE_PRIVATE).edit();
	...
	private synchronized void initSessionKey() {
		SharedPreferences sp = context.getSharedPreferences(RENREN_SDK_CONFIG, Context.MODE_PRIVATE);
	...
	private void exchangeSessionKey(String accessToken) {
		...
		try {
			...
			Log.i(Util.LOG_TAG, "---login success sessionKey:" + sessionKey + " expires:" + expires + " sessionSecret:"
					+ sessionSecret + " uid:" + uid);
	 ...
	 * @param uid
	 */
	private void storeSessionParams(String sessionKey, String sessionSecret, long sessionCreateTime, long expires,
			long uid) {
		if (sessionKey == null || sessionSecret == null) {
			this.clearPersistSession();
			return;
